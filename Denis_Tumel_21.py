...
# f=open('task_1.txt','a',encoding='utf-8') #создаем текстовый файл и открываем его
#
# for i in k:
#     f.write(str(i[0]))
#     f.write(str('   '))
...
# f.close() #закрывакм текстовый файл
#
#
# # 2. В БД из первого задания удалить первую ПОЛОВИНУ записей,
# a=int(len(k)/2)
# cursor.execute('''DELETE FROM TASK_1 WHERE ID<=?''',(k[a-1][0],))# удаляем первую  половину списка
# conn.commit()# сохроняем таблицу
# cursor.execute('''UPDATE TASK_1 SET col_2='goodbye' WHERE ID>?''',(k[a-1][0],))
# conn.commit()# сохроняем таблицу
...
# cursor.execute('''SELECT*FROM TEXT_1''')
# m=cursor.fetchall()
#
# if len(k)>5: #если в второй таблице число строк больше 5
#     cursor.execute('''DELETE FROM TEXT_1 WHERE col_1=?''', (m[0][0],)) # удаляем первую запись из первой таблицы
#     conn.commit()
# # Если меньше, то обновить первую запись в первой таблице на «hello»
# else:
#     cursor.execute('''UPDATE TEXT_1 SET col_1='hello' WHERE col_1=? ''',(m[0][0])) #редактирую первую запись в текстовой таблице
#     conn.commit()